...
- **Refine the three flag reasons** — today's flags are 🔒 Permanently closed / 📅 Not today / 🚫 Bad suggestion. Two need work: (1) "Not today" is ambiguous — it can mean a *date* signal (place is shut today / event not on) OR "we just can't go today" — split or clarify so the app can react correctly (e.g. a true closed-today could suggest a date to come back). (2) "Bad suggestion" is a dead-end — capture *why* (wrong vibe / too far / too pricey / not our thing) so it becomes real feedback that can shape future suggestions and feed the Playbill/learning loop. (Currently only "Permanently closed" persists; the rest are session-only.)
